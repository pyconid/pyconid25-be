...
from core.oauth_github_service import oauth_github_service
...
async def github_signin(http_request: Request, params: OauthSignInRequest = Depends()):
    try:
        authorization_url = await oauth_github_service.initiate_oauth(
...
async def github_verified(
    ...
    try:
        ...
        oauth_result = await oauth_github_service.handle_verified(
            request=http_request, db=db
        )
